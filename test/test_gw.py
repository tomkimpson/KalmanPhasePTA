...
"""Test the psr term calls ok"""
def test_psr_term():

    #Calls ok, no Nans
    P   = system_parameters.SystemParameters()  
    PTA = pulsars.Pulsars(P)         
    psr_term = gravitational_waves.gw_psr_terms(P.δ,P.α,P.ψ,PTA.q,PTA.q_products,P.h,P.ι,P.Ω,PTA.t,P.Φ0,PTA.χ)
    assert ~np.isnan(np.min(psr_term))
    ...
    P   = system_parameters.SystemParameters()  
    PTA = pulsars.Pulsars(P)         
    earth_term = gravitational_waves.gw_earth_terms(P.δ,P.α,P.ψ,PTA.q,PTA.q_products,P.h,P.ι,P.Ω,PTA.t,P.Φ0,PTA.χ)
    assert ~np.isnan(np.min(earth_term))


    #Calls ok, no Nans
    P   = system_parameters.SystemParameters()  
    PTA = pulsars.Pulsars(P)         
    null_term = gravitational_waves.null_model(P.δ,P.α,P.ψ,PTA.q,PTA.q_products,P.h,P.ι,P.Ω,PTA.t,P.Φ0,PTA.χ)
    assert np.all(null_term==0.0)
